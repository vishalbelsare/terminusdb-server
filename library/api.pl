...
% Database construction and accessors
...
document_handler(get, DB, Doc_ID, Request) :-
    ...
    try_make_db(DB_URI,DBObj),

    try_doc_uri(DB_URI,Doc_ID,Doc_URI),

    try_get_document(Doc_URI,DBObj,JSON),
...
document_handler(post, DB, Doc_ID, R) :-
    ...
    try_make_db(DB_URI, DB_Obj),
    
    try_get_param('terminus:document',Request,Doc),
        
    try_update_document(Doc_ID,Doc,DB_Obj),
...
document_handler(delete, DB, Doc_ID, Request) :-
    ...
    try_delete_document(Doc_URI,DB_Obj),
...
class_frame_handler(get, DB, Request) :-
    ...
    try_make_db(DB_URI,DB),

    try_get_param('terminus:class',Request,Class_URI),

    try_class_frame(Class_URI,DB,Frame),
 ...
 * http error otherwise.
 */
try_update_document(Doc_ID, Doc_In, DB) :-
...
/* 
 * try_make_db(+DB:uri,-DB:graph) is det. 
 * 
 * Die if we can't form a graph
 */
try_make_db(DB_URI,DB) :-
    (   make_database_from_database_name(DB_URI,DB)
    ->  true
...
/* 
 * try_create_db(DB_Name,DB_URI,Object) is det.
 * 
 */
try_create_db(DB_Name,DB_URI,Doc) :-
    with_mutex(
        DB_URI,
        (   (   create_db(DB_URI)
            ->  true
            ...
            (   add_database_resource(DB_Name,DB_URI,Doc)
            ->  true
            ;   (   delete_db(DB_URI)
...
/* 
 * try_class_frame(Class,DB,Frame) is det. 
 */ 
try_class_frame(Class,DB,Frame) :-
    (   class_frame_jsonld(Class,DB,Frame)
    ->  true
