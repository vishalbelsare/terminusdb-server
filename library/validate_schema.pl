...
%% collect(+collection:atom, +Database_ID:database_identifier, +X:uri_or_id, -L:any) is semidet.
%
...
collect(_,_,'http://www.w3.org/1999/02/22-rdf-syntax-ns#nil',[]) :-
    !.
collect(DatabaseAtom,X,[H|T]) :-
    xrdf(DatabaseAtom,X,'http://www.w3.org/1999/02/22-rdf-syntax-ns#first',H),
    !, % assume one result
    xrdf(DatabaseAtom,X,'http://www.w3.org/1999/02/22-rdf-syntax-ns#rest',Y),
    !, % assume one result
    collect(DatabaseAtom,Y,T).
...
%
% @param Schema,CC The class URI_OR_ID of which X isr.
...
oneOf(CC,X,Database) :-
    ...
    collect(Schema,ListObj,OneList),
    member(X,OneList).
...
anonymousEquivalentClass(C,CE,Database) :-
    ...
    (setof(X,xrdf(Schema,X,_,CE), ListX) *-> ListX = L ; L = []),
    length(L,1).
...
dcog_tag(X,Y,Database) :-
    database_schema(Database,Schema),
    xrdf(Schema,X, 'https://datachemist.net/ontology/dcog#tag', Y).
